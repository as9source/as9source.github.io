 ...
 *
 * 【強度についての注意】
 * ZipCryptoは既知平文攻撃に弱い。中身がPDFだと先頭が "%PDF-" と確定しているため、
 * 専用ツールがあればパスワードを知らなくても短時間で解読されうる。
 * したがってこれは「宛先を間違えて送ってしまったときに、すぐには中身を見られない」
 * という誤送信対策のレベルであり、機密保護（暗号学的な強度）を意味しない。
 * 受け取る側の環境を選ばない（追加ソフト不要）ことを優先した意図的な選択である。
 * 本当に秘匿が必要な情報は、この仕組みに頼らず別の手段で受け渡すこと。
 * 強度を上げる場合はAES-256（WinZip AES）化が必要だが、Windows標準・Mac標準では
 * 開けなくなり、受け取る側に7-Zip等の導入を求めることになる。
  ...
  function run(opts){
    ...
    return new Promise(function(resolve){
      ...
      function showPasswordState(){
        ...
        desc.textContent = 'PDFをこのパスワードで保護したZIPに変換してから送信します。相手には、このパスワードを電話やSMSなど別の方法で伝えてください。なお、この方式は宛先間違いに備えるためのもので、内容を完全に秘匿するものではありません。';
